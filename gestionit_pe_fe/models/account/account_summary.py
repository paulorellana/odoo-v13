...
class AccountSummary(models.Model):
    ...
    def load_summary(self):
        ...
        def get_tipo_doc_referencia(invoice):
            ...
            if invoice.journal_id.invoice_type_code_id == "07":
                tipo_documento = invoice.reversed_entry_id.invoice_type_code if invoice.reversed_entry_id else ""
                num_documento = invoice.reversed_entry_id.name if invoice.reversed_entry_id else ""

            elif invoice.journal_id.invoice_type_code_id == "08":
                tipo_documento = invoice.debit_origin_id.invoice_type_code if invoice.debit_origin_id else ""
                num_documento = invoice.debit_origin_id.name if invoice.debit_origin_id else ""

            return (tipo_documento,num_documento)
        ...
        account_summary_lines = [{
            ...
            "monto_exp":invoice.total_venta_exportacion,
            "monto_otros":0.0,
            "tipo_doc_referencia": get_tipo_doc_referencia(invoice)[0],
            "num_doc_referencia": get_tipo_doc_referencia(invoice)[1],
