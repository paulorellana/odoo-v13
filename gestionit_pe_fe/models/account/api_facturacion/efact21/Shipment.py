...
class HandlingInstructions(SimpleText):
    def __init__(self, handling_instructions):
        super(HandlingInstructions, self).__init__(handling_instructions, "cbc:HandlingInstructions")
...
class CarrierParty(Xmleable):
    ...
    def generate_name(self):
        elem = default_document.createElement("cac:PartyLegalEntity")
        elem_name = default_document.createElement("cbc:RegistrationName")
        elem_name.appendChild(default_document.createCDATASection(self.name))
        elem.appendChild(elem_name)
        return elem
...
class DriverPerson(Xmleable):
    def __init__(self, person_id, scheme_id="1",first_name=None,family_name=None,identity_document_reference=None):
    ...
    def generate_doc(self):
        ...
        if self.scheme_id:
            elem_id.setAttribute("schemeID", self.scheme_id)
            elem_id.setAttribute("schemeName", "Documento de Identidad")
            elem_id.setAttribute("schemeAgencyName", "PE:SUNAT")
            elem_id.setAttribute("schemeURI", "urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06")
        self.doc.appendChild(elem_id)

        if self.first_name:
            first_name = createElementContent("cbc:FirstName",self.first_name)
            self.doc.appendChild(first_name)

        if self.family_name:
            family_name = createElementContent("cbc:FamilyName",self.family_name)
            self.doc.appendChild(family_name)
    
        job_title = createElementContent("cbc:JobTitle","Principal")
        self.doc.appendChild(job_title)

        if self.identity_document_reference:
            id =  createElementContent("cbc:ID",self.identity_document_reference)
            identity_document_reference = default_document.createElement("cac:IdentityDocumentReference")
            identity_document_reference.appendChild(id)
            self.doc.appendChild(identity_document_reference)
...
# End Shipment Stage
class Despatch(Xmleable):
    def __init__(self, address_id, street_name,numero_anexo = None, numero_identificacion = None):
    ...
    def generate_doc(self):
        self.doc = default_document.createElement("cac:Despatch")
        da = default_document.createElement("cac:DespatchAddress")
        da.appendChild(createElementContent("cbc:ID", self.address_id))
        if self.numero_anexo and self.numero_identificacion:
            address_type_code = createElementContent("cbc:AddressTypeCode",self.numero_anexo)
            address_type_code.setAttribute("listID", self.numero_identificacion)
            da.appendChild(address_type_code)
        line = createElementContent("cbc:Line",self.street_name)
        address_line = default_document.createElement("cac:AddressLine")
        address_line.appendChild(line)

        da.appendChild(address_line)

        self.doc.appendChild(da)

class DeliveryAddress(Xmleable):
    def __init__(self, address_id, street_name,despatch,numero_anexo = None, numero_identificacion = None):
        self.address_id = address_id
        self.street_name = street_name
    ...
    def generate_doc(self):
        ...
        self.doc.appendChild(da)
        self.doc.appendChild(self.despatch.get_document())
...
class Shipment(Xmleable):
    def __init__(self, ship_id=None, handling_code=None, information=None, split_consignment_indicator=None,
                 gross_weight_measure=None, transport_handling_quantity=None, shipment_stage=None,handling_instructions=None,
    ...
    def fix_values(self):
        ...
        if type(self.handling_instructions) == str:
            self.handling_instructions = HandlingInstructions(self.handling_instructions)
    ...
    def validate(self, erros, observs):
        ...
        assert type(self.handling_instructions) == HandlingInstructions
