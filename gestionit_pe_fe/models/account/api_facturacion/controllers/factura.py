...
from ..efact21.DocumentReference import DespatchDocumentReference, DocumentTypeCode,AdditionalDocumentReference,IssuerParty
...
from ..efact21.PrepaidPayment import PrepaidPaymentID,PrepaidPayment
...
def build_factura(data):
    ...
    subTotalAnticipos = documento.get("subTotalAnticipos", 0.0)
    ...
    if "descuentoGlobal" in documento:
        ...
        if ("factor" in documento_descuento_global) and ("montoDescuento" in documento_descuento_global) and ("montoBase" in documento_descuento_global):
            descuento_global = AllowanceCharge(charge_indicator=False,
                                               ...
                                               base_amount=documento_descuento_global.get("montoBase"),
    ...
    if "retencion" in documento:
        ...
        if ("factor" in documento_retencion) and ("montoRetencion" in documento_retencion) and ("montoBase" in documento_retencion):
            retencion = AllowanceCharge(charge_indicator=False,
                                                ...
                                                amount=documento_retencion.get("montoRetencion"),
                                                base_amount=documento_retencion.get("montoBase"),
                                                currency_id=tipoMoneda,
                                                multiplier_factor_numeric=documento_retencion.get("factor", 0.0))  # Porcentaje de la retención
    allowance_charge_anticipo = None
    if mntTotalAnticipos > 0:
        allowance_charge_anticipo = AllowanceCharge(charge_indicator=False,
                                                    allowance_charge_reason_code="04",
                                                    amount=round(subTotalAnticipos,2),
                                                    base_amount=round(mntNeto + mntExe + mntExo + mntExportacion + subTotalAnticipos,2),
                                                    currency_id=tipoMoneda)
    ...
    line_extension_amount = LineExtensionAmount(amount=round( mntNeto + mntExe + mntExo + mntExportacion + subTotalAnticipos, 2), currencyID=tipoMoneda)
    ...
    tax_inclusive_amount = TaxInclusiveAmount(amount=round(mntNeto + mntExe + mntExo + mntTotalIgv +mntExportacion + mntTotalAnticipos, 2), currencyID=tipoMoneda)
    ...
    fact = Factura(ubl_extensions=None, ubl_version="2.1", id=id,
                   ...
                   allowance_charge_anticipo=allowance_charge_anticipo)
    ...
    for line in filter(lambda d: d.get("anticipoRegularizacion",False),detalle):
        anticipo_ID = line.get("anticipoDocumento","")
        
        if re.match("^F\w{3}[-]\d{1,8}$",anticipo_ID or ""):
            anticipo_type_code = "02"
        if re.match("^B\w{3}[-]\d{1,8}$",anticipo_ID or ""):
            anticipo_type_code = "03"
            
        anticipo_status_code = str(line.get("id"))
        anticipo_party_identification = PartyIdentification(numDocEmisor)
        anticipo_issuer_party = IssuerParty(party_identification=anticipo_party_identification)
        anticipo = AdditionalDocumentReference(id=anticipo_ID,
                                                document_type_code=anticipo_type_code,
                                                document_status_code=anticipo_status_code,
                                                issuer_party=anticipo_issuer_party)
        fact.add_additional_document_reference(anticipo)

        prepaid_payment_id = PrepaidPaymentID(anticipo_status_code)
        monto_total_linea = abs(line.get("montoTotalLinea",0))
        prepaid_payment = PrepaidPayment(prepaid_payment_id,monto_total_linea,tipoMoneda)
        fact.add_prepaid_payment(prepaid_payment)

    # LINEAS
    for line in filter(lambda d:not d.get("anticipoRegularizacion",False),detalle):
        ...
        if "descuento" in line:
            ...
            if "factor" in line_descuento and "montoDescuento" in line_descuento and "montoBase" in line_descuento:
                descuento = AllowanceCharge(charge_indicator=False,
                                            ...
                                            # base_amount es el monto total de la línea sin IGV y sin descuento
                                            base_amount=line_descuento.get("montoBase"),
                                            currency_id=tipoMoneda)
