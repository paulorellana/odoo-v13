...
class AccountMove(models.Model):
       ...
       for rec in self:
            ...
            if rec.invoice_type_code and rec.journal_ids and rec.journal_type and rec.warehouse_id and rec.type in ['out_invoice','out_refund']:
                domain =[
                    ...
                    ('id','in',[i.id for i in rec.warehouse_id.journal_ids]),
    ...
    @api.constrains("invoice_date_due", "paymentterm_line")
    ...
    is_invoice_regular_with_advanced_payments = fields.Boolean("Factura Regular con Pagos Anticipados",compute="_compute_is_invoice_regular_with_advanced_payments",store=True)
        ...
        if record.is_invoice_regular_with_advanced_payments:
            for invoice in self:
                ...
                for line in invoice.invoice_line_ids.filtered(lambda r: r.is_downpayment):
                    if order.exists():
                        lines = order.invoice_ids.filtered(lambda r:not r.is_invoice_regular_with_advanced_payments).mapped("invoice_line_ids").filtered(lambda r:r.price_subtotal + line.price_subtotal == 0 and r not in used)
                        if lines.exists():
                            line.anticipo_ref = lines[0].move_id.name
                            used.append(lines[0])
                


    #DESUSO
    invoice_type_code_catalog_51_code = fields.Char(related="invoice_type_code_catalog_51.code")
    ...
    def _onchange_recompute_dynamic_lines(self):
        ...
        for record in self:
            ...
            #if line_ids.filtered(lambda r:r.product_id.)
    ...
    total_anticipos = fields.Monetary(string="Total Anticipos",compute="_compute_amount")

    subtotal_anticipos = fields.Monetary(string="Subtotal Anticipos",compute="_compute_amount")
    ...
    def _compute_amount(self):
        ...
        for move in self:
            ...
            move.total_anticipos = abs(sum([line.price_total for line in move.invoice_line_ids if (line.is_downpayment) ]))
            
            move.subtotal_anticipos = abs(sum([line.price_subtotal for line in move.invoice_line_ids if (line.is_downpayment) ]))
    ...
    def validar_lineas(self):
        ...
        for line in self.invoice_line_ids.filtered(lambda r: not r.display_type):
            ...
            if line.quantity <= 0 and not line.is_downpayment:
                errors.append(
            ...
            if (line.price_subtotal == 0 or line.price_total==0) and not any([tax.tax_group_id.tipo_afectacion in ["31","32","33","34","35","36"] for tax in line.tax_ids]):
                errors.append("El precio unitario y la cantidad de los productos debe ser siempre mayor a 0. Revise el producto {} y cambie el precio a un valor mayor a 0.".format(line.name))
                break
    ...
    def cron_action_send_invoice(self):
        invoices = self.env["account.move"].search([["estado_emision", "in", ["P", "R", "N","", False]],
                                                    ["name", "not in",
                                                        [False, "/"]],
                                                    ["state", "=", "posted"],
    ...
    def action_download_cdr(self):
        ...
        if self.current_log_status_id:
            return {
                ...
                "url": "{}/my/invoices/{}/ei/cdr?access_token={}".format(web_base_url,self.id,self.access_token)
            }
        raise UserError("Aún no tiene registro de envío")

    def action_download_content_cdr(self):
        web_base_url =  self.env['ir.config_parameter'].sudo().get_param("web.base.url")
        if self.current_log_status_id:
            if self.current_log_status_id.response_content_xml:
                return {
                    "type":"ir.actions.act_url",
                    "url": "{}/my/invoices/{}/ei/cdr_content?access_token={}".format(web_base_url,self.id,self.access_token)
                }
        raise UserError("No se tiene el CDR aún.")
    ...
    def action_download_xml(self):
        ...
        if self.current_log_status_id:
            return {
                "type":"ir.actions.act_url",
                "url": "{}/my/invoices/{}/ei/xml?access_token={}".format(web_base_url,self.id,self.access_token)
            }
        raise UserError("Aún no tiene registro de envío")
    ...
    def onchange_template_id(self):
        for wizard in self:
            if wizard.composer_id:
                ...
                content_cdr_fname = invoice.name+"_content_cdr.xml"
                if invoice.current_log_status_id != False:
                    ...
                    response_content_xml = log_status.response_content_xml
                    if response_content_xml:
                        datas = base64.b64encode(response_content_xml.encode("ISO-8859-1"))
                        attach_ids.append(invoice.env["ir.attachment"].create(
                            {"name": content_cdr_fname, "type": "binary", "datas": datas, "mimetype": "text/xml", "res_model": "account.move", "res_id": invoice.id, "res_name": invoice.name}).id)
