...
class AccountLogStatus(models.Model):
    ...
    gre_ticket = fields.Char("Ticket GRE")
    gre_reception_date = fields.Datetime("Fecha de recepción envío de comprobante")
    gre_qr_link = fields.Char("QR Link")
    zip_file = fields.Binary("Zip File")
    zip_filename = fields.Char("Nombre de archivo Zip")

    cdr_file = fields.Char("CDR File")
    ...
    def action_request_status_invoice(self):
        ...
        if account_move_id:
            if account_move_id.state == "posted":
                self.sudo()._request_api_check_invoice_validity()

    def action_request_status_ticket_gre(self):
        result = self._request_content_status_ticket_gre()
        if result:
            self.update(result)
        ...
        if self.guia_remision_id:
            if self.guia_remision_id.state == 'validado':
                client = self.company_id.sunat_service_client_gre()
                service_gre_cdr_object = ServiceGRECDR(client)
                response_gre_cdr_object = service_gre_cdr_object.get(self.gre_ticket)

                self.update({
                    "gre_qr_link": response_gre_cdr_object.get_document_description(),
                    "response_json": json.dumps(response_gre_cdr_object.resume_json()),
                    "status": response_gre_cdr_object.get_status(),
                    "cdr_file": response_gre_cdr_object.arcCdr
                })"""
                #result = search_ticket_gre_api(self.gre_ticket)
                #self.update(result)

    def _request_content_status_ticket_gre(self):
        if self.gre_ticket:
            client = self.company_id.sunat_service_client_gre()
            service_gre_cdr_object = ServiceGRECDR(client)
            response_gre_cdr_object = service_gre_cdr_object.get(self.gre_ticket)
            return {
                "gre_qr_link": response_gre_cdr_object.get_document_description(),
                "response_json": json.dumps(response_gre_cdr_object.resume_json(), indent= 2),
                "status": response_gre_cdr_object.get_status(),
                "cdr_file": response_gre_cdr_object.arcCdr
            }
        else :
    ...
    def get_gre_qr_link(self):
        if self.guia_remision_id:
            if self.gre_qr_link:
                return self.gre_qr_link
            else:
                return ""
        else:
            raise UserError("Método solo disponible para accont.log.status asociado a una guía de remisión")
    ...
    def _get_token_validez_comprobante(self):
        client_id = self.env["ir.config_parameter"].sudo().get_param("sunat.validez.comprobante.client_id")
        client_secret = self.env["ir.config_parameter"].sudo().get_param("sunat.validez.comprobante.client_secret")
    ...
    def action_request_cdr(self):
        company = self.company_id
        GetStatusCDRObject = GetStatusCDR()
        GetStatusCDRObject.login(company.get_username_sunat(),company.sunat_pass)
        GetStatusCDRObject.set_parameters(rucComprobante= company.vat,
                                            tipoComprobante= self.account_move_id.journal_id.invoice_type_code_id,
                                            serieComprobante= self.account_move_id.name.split("-")[0],
                                            numeroComprobante= int(self.account_move_id.name.split("-")[1]))
        result = GetStatusCDRObject.fetch_cdr()
        _logger.info(result)
        if result.get("success",False):
            self.write({"cdr_file":result.get("content")})
