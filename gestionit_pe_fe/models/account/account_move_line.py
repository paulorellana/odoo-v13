...
class AccountMoveLine(models.Model):
    ...
    anticipo_ref = fields.Char(string="Factura Anticipo")
    is_downpayment = fields.Boolean("Es Anticipo",default=False,store=True,compute="_compute_is_downpayment")


    @api.depends("account_id")
    def _compute_is_downpayment(self):
        for record in self:
            record.is_downpayment = record.account_id.user_type_id in [self.env.ref("account.data_account_type_prepayments"),self.env.ref("account.data_account_type_current_liabilities")] and record.price_subtotal < 0

    """
    @api.depends("product_id")
    def _compute_anticipo_ref(self):
        for record in self:
            order = self.env["sale.order"].search([("name","=",self.move_id.invoice_origin)])
            if order.exists():
                lines = order.invoice_ids.filtered(lambda r:not r.is_invoice_regular_with_advanced_payments).mapped("invoice_line_ids").filtered(lambda r:r.price_subtotal + record.price_subtotal == 0 and not record.anticipo_ref)
                if lines.exists():
                    record.anticipo_ref = lines[0].move_id.name
            record.anticipo_ref = ""
    """
