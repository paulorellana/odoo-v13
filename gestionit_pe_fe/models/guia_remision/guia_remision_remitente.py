...
from odoo.addons.gestionit_pe_fe.models.guia_remision.send_gre_api import send_gre_api,search_ticket_gre_api
...
#patron_licencia = re.compile("^(?!0+$)[a-zA-Z0-9]{9,10}$")
patron_licencia = re.compile("^[a-zA-Z0-9]{9,10}$")
patron_placa_vehicular = re.compile("[0-9a-zA-Z]{3}[0-9]{3}|[0-9a-zA-Z]{3}-[0-9]{3}|[0-9a-zA-Z]{3}-[0-9]{2}[0-9a-zA-Z]$")

codigo_unidades_de_medida = ["DZN","DAY","HUR","LTR","NIU","CMT","GLL","OZI","GRM","GLL","KGM","LBR","MTR","LBR","SMI","ONZ","FOT","INH","LTN","BX","MTQ",
...
class GuiaRemisionLine(models.Model):
    ...
    guia_remision_id = fields.Many2one("gestionit.guia_remision", ondelete='cascade')
...
class GuiaRemision(models.Model):
    ...
    def generate_text_qr_guia_remision(self):
        if self.current_log_status_id:
            return self.current_log_status_id.get_gre_qr_link()
        return ""
    
        """
        ruc_emisor = self.company_partner_id.vat
    ...
    descripcion_motivo_traslado = fields.Char(string="Descripción de motivo de traslado")
    ...
    partner_id = fields.Many2one("res.partner", string="Destinatario", states={'validado': [('readonly', True)]})
    partner_tipo_documento_identidad = fields.Char(string="Tipo de Documento", related="partner_id.l10n_latam_identification_type_id.name")
    partner_numero_documento_identidad = fields.Char(string="Número de Documento", related="partner_id.vat")
    partner_direccion = fields.Char(string="Dirección", related="partner_id.street")
    partner_ubigeo = fields.Char(string="Ubigeo", related="partner_id.ubigeo")
    ...
    @api.onchange("partner_id","motivo_traslado")
        ...
        for record in self:
            ...
            if record.motivo_traslado in ['01', '09', '19']:
                ...
                if record.partner_id:
                    record.partner_direccion_llegada_id = record.partner_id.id
                record.partner_direccion_partida_id = record.company_partner_id.id
            # 02-COMPRA
            if record.motivo_traslado in ['02']:
                # record.direccion_partida_id = False
                if record.partner_id:
                    record.partner_direccion_partida_id = record.partner_id.id
            ...
            if record.motivo_traslado in ['04', '18']:
                record.partner_id = record.company_partner_id.id
                record.partner_direccion_llegada_id = record.company_partner_id.id
                record.partner_direccion_partida_id = record.company_partner_id.id
            # 09 -EXPORTACIÓN
            ...
            # 08 - IMPORTACIÓN
            if record.motivo_traslado in ['08'] and record.partner_id:
                record.partner_direccion_partida_id = record.partner_id.id
                record.partner_direccion_llegada_id = record.partner_id.id

            if record.motivo_traslado == '13':
            ...
            if record.documento_asociado == "comprobante_pago":
                ...
                for comprobante_pago in record.comprobante_pago_ids:
                    guia_remision_lines += [{
                                            ...
                                            } for inv_line in comprobante_pago.invoice_line_ids if inv_line.quantity > 0]
    ...
    def generar_comprobante_json(self):
        ...
        if self.motivo_traslado in ["02","04","07"]:
            destinatario = self.company_partner_id
        elif self.motivo_traslado in ['01', '03', '05', '06', '09', '14' ,'17','13']:
        ...
        if self.multiple_tramos:
            ...
            entregaAnexo = self.direccion_llegada_id.numero_anexo or ""

            salidaUbigeo = self.lugar_partida_ubigeo_code.code
            salidaDireccion = str(self.lugar_partida_direccion or "").strip()[:98]
            salidaAnexo = self.direccion_partida_id.numero_anexo or ""
            
            partidaRucEstablecimiento = self.direccion_partida_id.vat or "" if self.direccion_partida_id else ""
            llegadaRucEstablecimiento = self.direccion_llegada_id.vat or "" if self.direccion_llegada_id else ""
        documento = {
            "serie": serie,
            ...
            "motivoTraslado": self.motivo_traslado,
            "descripcionMotivoTraslado":  self.descripcion_motivo_traslado if self.motivo_traslado == "13" else motivo_traslado_id.name.strip(),
            ...
            "entregaAnexo": entregaAnexo,
            "partidaRucEstablecimiento":partidaRucEstablecimiento,
            "salidaUbigeo": salidaUbigeo,
            "salidaDireccion": salidaDireccion,
            "salidaAnexo": salidaAnexo,
            "llegadaRucEstablecimiento":llegadaRucEstablecimiento
        }
        ...
        if not self.multiple_tramos:
            ...
            # Transporte Privado
                ...
                transportes.append({
                    ...
                    "nombreConductor": self.conductor_privado_partner_id.name,
                    "licenciaConductor": self.conductor_privado_partner_id.licencia,
                    "apellidosConductor": self.conductor_privado_partner_id.name,
            ...
            for line in self.transporte_lines:
                ...
                # Transporte Privado
                    ...
                    transportes.append({
                        ...
                        "nombreConductor":  line.conductor_privado_partner_id.name,
                        "apellidosConductor":  line.conductor_privado_partner_id.name,
                        "fechaInicioTraslado": str(line.date),
                        "placaVehiculo": line.vehiculo_privado_id.numero_placa,
                        "modoTraslado": line.modalidad_transporte,
                        "licenciaConductor": line.licencia,
                    })
        ...
        data = {
            "company": {
                ...
                "SUNAT_user": self.company_id.sunat_user_sol,
                "SUNAT_pass": self.company_id.sunat_pass_sol,
    ...
    def validar_transporte(self):
        ...
        if self.modalidad_transporte == "01":
            ...
            if not self.conductor_privado_partner_id:
                ...
                if self.conductor_privado_partner_id.l10n_latam_identification_type_id.l10n_pe_vat_code == "6":
                    errors.append("* El conductor privado no puede tener RUC.")

                if not self.fecha_inicio_traslado:
                    errors.append("* La fecha de inicio de traslado es obligatorio")

                if not bool(patron_licencia.match(self.conductor_privado_partner_id.licencia or "")):
                    errors.append("* La licencia del conductor seleccionado no es valida o no tiene. Recuerde que la licencia es un alfanumérico de 9 a 10 carácteres.")
                
            if not self.vehiculo_privado_id:
                errors.append("* El Vehículo privado es Obligatorio.")
            elif not bool(patron_placa_vehicular.match(self.vehiculo_privado_id.numero_placa or "")):
                errors.append("* El Vehículo pivado seleccionado no tiene Número de Placa o tiene un formato inválido")

        return errors

    
    def validar_partida_llegada(self):
        errors = []
        if self.direccion_partida_id == self.direccion_llegada_id:
            errors.append("* La dirección de partida y llegada no pueden ser las mismas.")
        return errors
    ...
    def validar_restricciones(self):
        ...
        errors += self.validar_partida_llegada()
        ...
        if self.multiple_tramos:
            ...
            elif len(self.transporte_lines) > 1:
                if self.transporte_lines.mapped("modalidad_transporte") == 1:
                    errors.append("* Solo se puede emitir guías de remisión remitente de tramos privados. ")
            else:
                for line in self.transporte_lines:
    ...
    def generar_log_envio(self):
        ...
        log_status = {
            ...
            # "status": "P",
            "zip_file":request.get("zip_file",""),
            "zip_filename":request.get("zip_filename",""),
            "digest_value": request.get("digest_value", "-"),
            "signed_xml_data": parseString(request.get("signed_xml")).toprettyxml(" ") if request.get("signed_xml", False) else "",
            "signed_xml_with_creds": parseString(request.get("final_xml")).toprettyxml(" ") if request.get("final_xml", False) else "",
            "signed_xml_data_without_format":request.get("signed_xml", "-"),
    ...
    def request_status_ticket_gre(self,ticket):
        return search_ticket_gre_api(self,ticket)


    def action_request_status_ticket_gre(self):
        self.current_log_status_id.action_request_status_ticket_gre()

    def send_gr_xml(self):
        # if not self.current_log_status_id:
        self.generar_log_envio()
        # try:
        result = send_gre_api(self,tipo_envio=self.company_id.tipo_envio)
        if "gre_ticket" not in result:
            return {}
        result.update(self.request_status_ticket_gre(result["gre_ticket"]))
    ...
    def cron_enviar_comprobante(self, cantidad):
        ...
        guia_remision_ids = self.env["gestionit.guia_remision"].search(['&','|',("current_log_status_id","=",False),("current_log_status_id.status", "in", ["E","P"])
                                                                            ,'&',("state","=","validado"),("fecha_emision",">=",date)], limit=cantidad,order="create_date desc")
        for gr in guia_remision_ids:
            try:
                if gr.current_log_status_id:
                    gr.action_request_status_ticket_gre()
                if gr.current_log_status_id.status != "A":
                    gr.send_gr_xml()
                self.env.cr.commit()
            except Exception as e:
                _logger.info(gr.numero)
                pass
