...
class SalePaymenttermLine(models.Model):
    ...
    date_due = fields.Date("Fecha de vencimiento estimada",required="True")
...
class SaleOrder(models.Model):
    ...
    paymentterm_line = fields.One2many("sale.paymentterm.line", "order_id",string="Cuotas")
    ...
    amount_untaxed_mn = fields.Monetary(string="Base Imponible MN",
        compute="compute_campo_amount_untaxed_mn",store=True,currency_field='company_currency_id')
    amount_tax_mn = fields.Monetary(string="Impuestos MN",
        compute="compute_campo_amount_tax_mn",store=True,currency_field='company_currency_id')
    amount_total_mn = fields.Monetary(string="Total en MN",
        compute="compute_campo_amount_total_mn",store=True,currency_field='company_currency_id')
    ##########################################################################
    ...
    @api.depends('total_venta_gravado','currency_rate')
    def compute_campo_amount_untaxed_mn(self):
        for rec in self.sudo():
            rec.amount_untaxed_mn = rec.total_venta_gravado/(rec.sudo().currency_rate or 1)


    @api.depends('total_igv','currency_rate')
    def compute_campo_amount_tax_mn(self):
        for rec in self.sudo():
            rec.amount_tax_mn = rec.total_igv/(rec.sudo().currency_rate or 1)


    @api.depends('amount_total','currency_rate')
    def compute_campo_amount_total_mn(self):
        for rec in self.sudo():
            rec.amount_total_mn = rec.amount_total/(rec.sudo().currency_rate or 1)
    ...
    def _get_date_due(self):
        for record in self:
            if record.payment_term_id and record.date_order:
                try:
                    record.date_due = record.payment_term_id.compute(0, date_ref=record.date_order, currency=record.company_id.currency_id)[0][0]
    ...
    @api.constrains('paymentterm_line')
    ...
    def _compute_residual_credit_paymentterm(self):
        for order in self:
            order.residual_credit_paymentterm = round(self.amount_total - sum(order.paymentterm_line.mapped("amount")), 2)
    ...
    tipo_documento = fields.Selection(string="Tipo de Documento CPE",
