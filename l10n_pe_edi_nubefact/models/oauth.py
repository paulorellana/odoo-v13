...
import zipfile
from odoo.addons.gestionit_pe_fe.models.account.api_facturacion.controllers import xml_validation, main, firma
from odoo.addons.l10n_pe_edi_nubefact.models.api_facturacion.controllers import sunat_response_handle
...
def send_summary_xml(doc):
    ...
    elif doc.company_id.sunat_provider == "nubefact":
        if int(tipo_envio) == 0:
            endpoint = urls_test_nf[0]

        elif int(tipo_envio) == 1: 
            endpoint = urls_production_nf[0]
        else:
            raise Exception("Tipo de envio incorrecto. Tipos de envío posibles: 0 - Pruebas , 1- Producción")
...
def send_doc_xml(doc):
    ...
    if doc.company_id.sunat_provider == "sunat":
        ...
        # _logger.info("-->> nubefact")
        if int(tipo_envio) == 0:
            if invoice_type_code in ["01","03","07","08","RC"]:
                endpoint = urls_test_nf[0]
            elif invoice_type_code == "09":
                endpoint = urls_test_nf[1]
        elif int(tipo_envio) == 1: 
            if invoice_type_code in ["01","03","07","08","RC"]:
                endpoint = urls_production_nf[0]
            elif invoice_type_code == "09":
                endpoint = urls_production_nf[1]
        else:
            raise Exception("Tipo de envio incorrecto. Tipos de envío posibles: 0 - Pruebas u 1- Producción")
    
    # _logger.info("send_doc_xml; endpoint: %s" % endpoint)
...
def crear_json_fac_bol(self):
    ...
    data = {
        ...
        "documento": {
            ...
            "subTotalAnticipos": round(self.subtotal_anticipos,2),
            "mntTotalAnticipos": round(self.total_anticipos,2),
    ...
    if self.apply_retention:
        data["documento"]["retencion"] = {
            ...
            "montoBase": round(self.amount_total,2)
        }
    ...
    for item in self.invoice_line_ids.filtered(lambda r: not r.is_charge_or_discount and not r.display_type):
        ...
        datac = {
            ...
            "precioItem": round(item.price_total/item.quantity, 4) if not any([tax.tax_group_id.tipo_afectacion in ["31", "32", "33", "34", "35", "36"] for tax in item.tax_ids]) else 0,
            # Precio unitario sin IGV y sin descuento
            "precioItemSinIgv": round((item.price_subtotal/item.quantity)/(1-item.discount/100), 4) if not any([tax.tax_group_id.tipo_afectacion in ["31", "32", "33", "34", "35", "36"] for tax in item.tax_ids]) else 0,
            ...
            "montoTotalLinea":round(item.price_total,2),
            ...
            "anticipoRegularizacion": bool(item.is_downpayment and re.match("^[F|B]\w{3}[-]\d{1,8}$",item.anticipo_ref or "")),
            "anticipoDocumento": item.anticipo_ref if item.is_downpayment and re.match("^[F|B]\w{3}[-]\d{1,8}$",item.anticipo_ref or "") else "",
        }
        if item.discount:
            datac["descuento"] = {
...
def crear_json_not_cred_deb(self):
    ...
    for item in self.invoice_line_ids.filtered(lambda r: not r.is_charge_or_discount and not r.display_type):
        ...
        datac = {
            ...
            "precioItem": round(item.price_total/item.quantity, 4) if len([item for line_tax in item.tax_ids if line_tax.tax_group_id.tipo_afectacion in ["31", "32", "33", "34", "35", "36"]]) == 0 else 0,
            ...
            "precioItemSinIgv": round((item.price_subtotal/item.quantity), 4) if len([item for line_tax in item.tax_ids if line_tax.tax_group_id.tipo_afectacion in ["31", "32", "33", "34", "35", "36"]]) == 0 else 0,
            # "montoItem": round(item.price_subtotal, 2),
            "montoItem": round(item.price_unit*item.quantity, 2) if item.no_onerosa else round(item.price_subtotal, 2),
