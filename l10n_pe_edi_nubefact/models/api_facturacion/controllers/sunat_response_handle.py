...
def get_response(xml_response):
    ...
    # _logger.info("faultcodes: %s" % faultcodes)
    # _logger.info("applicationResponse: %s" % applicationResponse)
    # _logger.info("faults: %s" % faults)
    ...
    if faultcodes:
        ...
        # _logger.info("-->> get_response - faults")
        is_nubefact = False
        detail = ""
        for i in range(faults.length):
            code = faults.item(i).getElementsByTagName("faultcode")[0].firstChild.data
            ...
            elif code ==  "soap-server":
                is_nubefact = True
                code = faults.item(i).getElementsByTagName("faultstring")[0].firstChild.data
                code = int(code)
                detail = faults.item(i).getElementsByTagName("detail")[0].childNodes[0].firstChild.data
                detail1 = faults.item(i).getElementsByTagName("detail")[0].getElementsByTagName("message").firstChild.data
                # _logger.log("-->> detail: %s" % detail)
                # _logger.log("-->> detail1: %s" % detail1)
            else:
                code = int(code.split(":")[1])
